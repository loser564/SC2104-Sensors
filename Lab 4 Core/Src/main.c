...
int main(void)
{
  ...
  // LAB: change Kp and observe changes to system
  Kp = 6;   // range: 1 to 10
  Ki = 2;     // range 0 to 3
  Kd = 1;   // range: 0 to 3
  // can increase kp until ringing at output occurs
  // add kd and increase until ringing stops
  // add ki and adjust output value towards target value
  ...
  while (1){
  	  ...
  	  // LAB: comment out in 3.2 (b)
  	  pwmVal = PID_Control(); // call the PID control loop calculation
  	  // LAB: change this value to find the minimum value for the motor to rotate
